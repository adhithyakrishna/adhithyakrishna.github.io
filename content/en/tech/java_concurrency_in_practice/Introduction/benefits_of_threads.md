...
#### Benefits of threads
Proper utilization of threads can:
1) Reduce development and maintenance cost.
2) Improve the performance of complex applications.

Threads are useful in GUI applications for improving the responsiveness of the user interface. In server applications, threads help improve resource utilization and throughput.
Since the **basic unit of scheduling** is the thread, a single-threaded program can run on at most one processor at a time. When a single-threaded program runs on a 100-processor system, it is giving up access to 99%.
...
Assigning a thread to **each type of task** will make it look like it's sequential and insulates domain logic from the details of:
1) Scheduling
