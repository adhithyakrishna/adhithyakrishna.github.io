...
Concurrency is more about achieving thread-safety than it is about creating and managing threads. Those are mechanisms, but at its core, concurrency aims to encapsulate shared mutable state from uncontrolled concurrent access.
...
An object's state may include fields from other dependent objects. For example, a `HashMap`'s state is partially stored in the HashMap object itself, but also in many `Map.Entry` objects.
...
{{< /boxmd >}}
It is easier to design a class to be thread-safe than to retrofit it for thread safety later.

Object-oriented techniques can help write well-organized, maintainable classes—such as encapsulation and data hiding—and can also help create thread-safe classes.
...
{{< boxmd >}}
When designing thread-safe classes, good object-oriented techniques—encapsulation, immutability, and clear specification of invariants—are helpful.

An invariant is a condition that must remain true regardless of what changes occur or who uses/transforms the object.
{{< /boxmd >}}

There might be times when good OO design techniques are at odds with real-world requirements:
1) Rules of good design need to be compromised for the sake of performance or for the sake of backward compatibility with legacy code.
...
No set of operations performed sequentially or concurrently on instances of a thread-safe class can cause an instance to enter an invalid state.
...
It is only when a servlet wants to remember things from one request to another that thread safety becomes an issue.
...
In this example, a variable `count` is introduced to keep track of the number of requests processed. Though this may work fine in a single-threaded environment, it is susceptible to **lost updates**.
The `count` variable, as noted previously, despite its shorthand syntax, has 3 discrete operations (read-modify-write):
...
If two threads increment the counter simultaneously without synchronization, the counter would not give an accurate result. The possibility of incorrect results in the presence of unfortunate timing is called a **race condition**.
...
{{< boxmd >}}
Where practical, use existing thread-safe objects, like `AtomicLong`, to manage a class's state. It is simpler to reason about the states and state transitions for existing thread-safe objects than for arbitrary state variables, and it makes it easier to maintain and verify thread safety.
{{< /boxmd >}}
#### Locking
...
When a thread requests a lock that is already held by another thread, the requesting thread blocks. However, since **intrinsic** locks are re-entrant, if a thread tries to acquire a lock that it already holds, the request succeeds.
Reentrancy is implemented by associating each lock with an acquisition count and an owning thread.
When a thread acquires a previously unheld lock, the JVM records the owner and sets the acquisition count to 1. If the same thread acquires the lock again, the count is incremented. When the **owning thread exits the synchronized block**, the count is decremented. When the count becomes zero, the lock is released.
...
Because locks enable serialized access (serializing access means threads take turns accessing the object exclusively, rather than accessing it concurrently) to the code paths they guard, we can use them to construct protocols for guaranteeing exclusive access to the shared state.

Compound actions on shared state, such as incrementing a hit counter (read-modify-write) or lazy initialization (check-then-act), must be atomic to avoid race conditions.
Just wrapping the compound action with a `synchronized` block is not sufficient:
1) If synchronization is used to coordinate access to a variable, it must be used everywhere that variable is accessed.
...
A common locking convention is to encapsulate all mutable state within an object to protect it from concurrent access by synchronizing any code path that accesses mutable state using the object's intrinsic lock. It is easy to subvert this locking protocol accidentally by adding a new method or code path and forgetting to use synchronization.

Adding synchronization to every method is not enough. For example, a `put-if-absent` operation has a race condition even though both `contains` and `add` are individually atomic when multiple operations are combined into a compound action.
...
It is easy to improve the concurrency of a component while maintaining thread safety by **narrowing the scope of synchronized blocks**. However, the scope of synchronized blocks should not be too small—an atomic operation should not be divided into more than one synchronized block. 
...
```
By narrowing the scope of synchronized blocks, the servlet's concurrency can be improved while maintaining thread safety.

The above example uses two synchronized blocks, each limited to a short section of code:
1) One guards the `check-then-act` that tests whether we can just return the cached result.
2) The other guards the update of the cached number and the cached factors.
